...
create policy if not exists "Founders can manage leads access"
...
-- Allow users to view their own access status (so they can detect access on the client)
create policy if not exists "Users can view their own leads access"
  on public.leads_access
  as permissive
  for select
  to authenticated
...
create policy if not exists "View leads if founder, creator, or granted"
  on public.leads
  for select
  to authenticated
  using (
...
-- Restrict INSERT to founder or granted employees (still must set created_by = auth.uid())
drop policy if exists "Users can create leads" on public.leads;
create policy if not exists "Founder or granted users can create leads"
  on public.leads
  for insert
  to authenticated
  with check (
...
-- Keep existing update/delete policies as-is
